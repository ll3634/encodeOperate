#!/usr/bin/env python3
"""
Analysis + report generator for the meeting-scheduling sanity check.

Reuses statistical machinery (paired McNemar + bootstrap CI) from the
billing analyzer; writes a meeting-specific report.md.
"""
...
def write_report(out_md: Path, config: dict, rates: dict,
    ...
    lines = []
    lines.append("# Meeting-scheduling sanity check (calendar-anchoring)")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append(
        "This is a behavior-only non-QA sanity check. The agent must schedule a "
        "30-minute 1:1 between two people. After one tool observation containing "
        "person A's calendar, it must either query person B's calendar or commit "
        "to a slot. Conditions:"
    )
    lines.append("")
    lines.append("- **N0**: A's calendar only. No anchor note.")
    lines.append("- **T0**: A's calendar + a 'their last 1:1 was at W' anchor note (the trap).")
    lines.append("- **IC**: A's calendar + a length-matched irrelevant note.")
    lines.append("- **S0**: A's AND B's calendars; W is the unique mutually-free slot.")
    lines.append("")
    lines.append(
        "Primary metric `commit_W` = the model finalizes a SLOT exactly equal to W. "
        "The W slot appears in A's free-slot list in *all* conditions, so commit_W "
        "is not driven by surface novelty of the slot string."
    )
    lines.append("")
    ...
    for bl in [
        ...
        "Calendar agents in production fail this exact way at this exact rate.",
        "The circuit identified in the QA experiments is the same circuit firing here.",
        "The model cannot do calendar reasoning at all.",
