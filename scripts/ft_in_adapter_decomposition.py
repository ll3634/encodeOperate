#!/usr/bin/env python3
"""
Audit 4 — Decomposition under a fine-tuned LoRA adapter (M1 vs M2).

Runs the §8.3 functional-decomposition margin-shift protocol UNDER an
adapter-loaded Qwen2.5-7B-Instruct. Mirrors `scripts/decomposition_ci_null.py`
but loads the adapter and accepts adapter-specific evidence/action directions
(parallel/perp components).

Outputs into <output_dir>:
  per_example_shifts.{npz,jsonl}
  bootstrap_cis.json
...
Usage:
  python scripts/ft_in_adapter_decomposition.py \
      --adapter-path adapters/qwen_balanced_v1 \
      --adapter-tag balanced \
      --dir-full     steering/directions/direction_decomp_full_layer20_ft.npz \
      --dir-parallel steering/directions/direction_decomp_parallel_layer20_ft.npz \
      --dir-perp     steering/directions/direction_decomp_perp_layer20_ft.npz \
      --output-dir   results/ft_in_adapter_d4/balanced_margin_shift \
      --n-samples 100 --n-random 20
"""
...
def bootstrap_ci(values, n_boot=N_BOOT, ci=95.0, seed=SEED):
    ...
    return {"mean": float(values.mean()),
            ...
            "n": int(n), "ci_low": lo, "ci_high": hi, "ci_level": ci,
            "boot_mean": float(boot_means.mean()),
            "boot_std": float(boot_means.std(ddof=1))}


def paired_perm(a, b, n_perm=N_PERM, seed=SEED):
    a = np.asarray(a, dtype=np.float64); b = np.asarray(b, dtype=np.float64)
    diff = a - b; n = len(diff)
    obs = float(np.abs(diff.mean()))
    rng = np.random.RandomState(seed)
    ...
    return {"mean_a": float(a.mean()), "mean_b": float(b.mean()),
            ...
            "n_perm_ge_observed": int(ge), "p_value": float(p)}


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--adapter-path", required=True)
    ap.add_argument("--adapter-tag", required=True)
    ap.add_argument("--dir-full", required=True)
    ap.add_argument("--dir-parallel", required=True)
    ap.add_argument("--dir-perp", required=True)
    ...
    print(f"[init] adapter={args.adapter_tag}  L{LAYER}  rho={RHO}  seed={SEED}"
          f"  N={args.n_samples}  K_random={args.n_random}")

    full_dir, _ = load_direction(args.dir_full,     normalize_rms=1.0)
    par_dir,  _ = load_direction(args.dir_parallel, normalize_rms=1.0)
    perp_dir, _ = load_direction(args.dir_perp,     normalize_rms=1.0)
    ...
    t0 = time.time(); n_total_fw = n * (4 + K); fw_done = 0
    ...
    rand_per_prompt = rand_sh.mean(axis=0)
    rand_per_dir_means = rand_sh.mean(axis=1)
    rand_per_dir_abs_means = np.abs(rand_sh).mean(axis=1)
    ...
    with open(os.path.join(args.output_dir, "per_example_shifts.jsonl"), "w") as f:
        for i in range(n):
            f.write(json.dumps({"sample_id": sample_ids[i],
    ...
    bootstrap = {"full":         bootstrap_ci(full_sh, N_BOOT, 95.0, SEED),
    ...
    with open(os.path.join(args.output_dir, "bootstrap_cis.json"), "w") as f:
        json.dump({"n_boot": N_BOOT, "ci_level": 95.0, "seed": SEED,
                   "n_samples": int(n), "bootstrap": bootstrap}, f, indent=2)

    null = {"K": int(K), "n_samples_per_dir": int(n),
            "signed": {"mean": float(rand_per_dir_means.mean()),
            ...
            "abs":   {"mean": float(rand_per_dir_abs_means.mean()),
    ...
    test_a = paired_perm(par_sh,  rand_per_prompt, N_PERM, SEED + 10)
    test_b = paired_perm(perp_sh, full_sh,         N_PERM, SEED + 11)
    test_c = paired_perm(perp_sh, par_sh,          N_PERM, SEED + 12)
    ...
    test_d = {"observed_gap_perp_minus_par": obs_gap,
              ...
              "n_perm": int(N_PERM), "n_perm_ge_observed_abs": int(ge_d),
              "p_value": (ge_d + 1) / (N_PERM + 1)}
    pairwise = {"n_perm": int(N_PERM), "n_pairs": int(n), "seed": SEED,
    ...
    summary = {"config": {"adapter_path": args.adapter_path,
                          "adapter_tag": args.adapter_tag,
                          "layer": LAYER, "rho": RHO, "seed": SEED,
                          "n_samples": int(n), "K_random": int(K),
                          "model": args.model,
                          "directions": {"full": args.dir_full,
    ...
    print(f"[done] full Δ = {full_sh.mean():+.3f}"
          f"  par Δ = {par_sh.mean():+.3f}"
          f"  perp Δ = {perp_sh.mean():+.3f}"
          f"  rand[K={K}] = {null['signed']['mean']:+.3f}"
          f"  null95 [{null['signed']['p2_5']:+.3f},{null['signed']['p97_5']:+.3f}]")
